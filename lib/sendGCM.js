...
const DEFAULT_TTL = _require.DEFAULT_TTL,
      GCM_METHOD = _require.GCM_METHOD;


const ttlFromExpiry = expiry => expiry - Math.floor(Date.now() / 1000);

const extractTimeToLive = R.cond([[R.propIs(Number, 'expiry'), ({ expiry }) => ttlFromExpiry(expiry)], [R.propIs(Number, 'timeToLive'), R.prop('timeToLive')], [R.T, R.always(DEFAULT_TTL)]]);
